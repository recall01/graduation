...
public class ChangeIcon extends View {
    ...
    private String lText = "签到";      //定义默认文字
    //定义文字大小
    private int lTextSize = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 12, getResources().getDisplayMetrics());
     ...
     * 获取自定义属性的值
    */
    public ChangeIcon(Context context, AttributeSet attrs, int defStyleAttr)
    {
        super(context, attrs, defStyleAttr);
        //获取自定义控件所有属性值
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.ChangeIcon);
    ...
    /**
     * 设置图标绘制位置|测量：EXACTLY,AT_MOST,UNSPECIIED
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec)
    {
        ...
        //获取图标宽度AT_MOST模式
/*        int mode = MeasureSpec.getMode(heightMeasureSpec);  //获取模式
        int size = MeasureSpec.getSize(heightMeasureSpec);  //获取高度  */
